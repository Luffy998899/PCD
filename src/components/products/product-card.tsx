...
export function ProductCard({
  ...
  return (
    <Card interactive className={cn('relative flex flex-col overflow-hidden', className)}>
      <div className="relative aspect-4/3 w-full border-b border-border bg-surface-subtle">
        {imageUrl ? (
          <Image
            src={imageUrl}
      ...
      <CardBody className="flex flex-1 flex-col gap-1.5">
        ...
        {product.strength || product.pack_size ? (
          <p className="text-sm text-muted-foreground">
            {[product.strength, product.pack_size].filter(Boolean).join(' · ')}
          </p>
        ) : null}
